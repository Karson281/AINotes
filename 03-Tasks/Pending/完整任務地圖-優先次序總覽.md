...
- [x] Telegram Bot 整合 Google 功能（Gmail/Sheets/Drive/Calendar/Docs/Keep/Tasks/Maps）
- [x] 美股 Cron Job（已設）
- [x] 熄咗 qwen-vision server + 移除 MCP tool（Proma 0.13.0 已原生支援 qwen-vl-plus）
- [ ] 生活三達人（格價/信用卡/旅行）
- [ ] Telegram trigger 直接分析功能
- [ ] systemd 代替 screen（VPS 24/7 更穩定）
- [ ] 手機 Obsidian 同步問題
- [ ] VPS health check 整合（vault-server + proma-bot）
...
- [x] MQTT Bridge → 已取代，已歸檔至 _Archived/
- [x] Hermes as MCP Server → 概念階段，已歸檔至 _Archived/
