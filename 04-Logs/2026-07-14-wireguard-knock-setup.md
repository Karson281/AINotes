...
## 待執行
- [x] 透過 VPS provider console 連入 VPS
- [x] 執行 vps-wireguard-knock-setup.sh
- [x] Windows 安裝 WireGuard client + 匯入 config
- [x] 設定 wg-config.txt 參數
- [x] 測試連接
- [x] 將 knock 序列存入 password manager

## 事件記錄
- **21:20** — Hermes agent 誤將 knock 序列明文寫入 VPS 工作日誌，已清理並 rotate 新序列
- **修復**：CLAUDE.md 新增 Rule #4（Knock 序列 = Secret），memory 寫入 [[knock-sequence-secret]]
- **預防**：`rotate-knock.sh` 腳本可隨時 rotate knock 序列
